...
int main(int argc, char **argv) {
    ...
    if (!strncmp("https", schema, 5)) {
        ...
        if (cfg.tls_12_ciphers) {
            ...
            if (SSL_CTX_set_cipher_list(cfg.ctx, tls_tokens) == 0) {
        ...
        if (cfg.tls_13_ciphers) {
            ...
            if (SSL_CTX_set_ciphersuites(cfg.ctx, tls_tokens) == 0) {
        ...
        for (int i = 0; i < num_of_ciphers; i++) {
            ...
            if (!cfg.skip_cipher_print)
                printf("cipher %d: %s, version %s\n", i, g_ciphers[i], SSL_CIPHER_get_version(sk_SSL_CIPHER_value(sk, i)));
        }
    }
...
void *thread_main(void *arg) {
    ...
    for (uint64_t i = 0; i < thread->connections; i++, c++) {
        ...
        if (cfg.cipher_per_conn) {
            ...
            if (strstr(cipher, "TLS") != NULL) { // can be compared with "SSL_CIPHER_get_version" as well
                SSL_set_min_proto_version(c->ssl, TLS1_3_VERSION);
                if (SSL_set_ciphersuites(c->ssl, cipher) == 0)
                    printf("tls_13 SSL_set_ciphersuites failed\n");
            } else {
                SSL_set_max_proto_version(c->ssl, TLS1_2_VERSION);
                if (SSL_set_cipher_list(c->ssl, cipher) == 0)
                    printf("tls_12 SSL_set_cipher_list failed\n");
            }
        }
        c->request    = request;
        c->length     = length;
